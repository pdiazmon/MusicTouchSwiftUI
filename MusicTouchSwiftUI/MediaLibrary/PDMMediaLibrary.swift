...
public class PDMMediaLibrary: MediaLibraryProtocol {
	...
	/// Returns all the songs in a given playlist and album
	/// - Parameters:
	///   - byPlaylist: Playlist name
	///   - byAlbumPersistentID: Album persistent ID
	/// - Returns: A MPMediaItem array with all the songs in the given playlist
	public func getSongsList(byPlaylist: String, byAlbumPersistentID: MPMediaEntityPersistentID) -> [MPMediaItem] {
		
		// Set the filter for the playlist name
		let playlistFilter	= MPMediaPropertyPredicate(value: byPlaylist, forProperty: MPMediaPlaylistPropertyName)
		let albumFilter		= MPMediaPropertyPredicate(value: byAlbumPersistentID,
		...
		let myFilterSet: Set<MPMediaPropertyPredicate> = [playlistFilter, albumFilter, songFilter]
